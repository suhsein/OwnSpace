...
인스턴스를 생성하고, 원격 DB에 연결하고, SSH로 원격 인스턴스에 연결해 빌드한 파일을 실행시켜서 임시배포 사이트를 실행하는 전 과정에 대해서 학습할 수 있었다.<br>
임시배포를 해보며 생각보다 IDE에서 처리해주는 일들이 훨씬 많다는 것을 알았다. 배포를 고려해서 더 주의깊게 코딩해야겠다는 깨달음을 얻었다.
